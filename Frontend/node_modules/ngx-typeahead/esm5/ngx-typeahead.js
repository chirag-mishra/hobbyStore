...
var NgxTypeAheadComponent = (function () {
    ...
    function (obs) {
        var _this = this;
        return obs
            .pipe(filter(function (e) { return validateNonCharKeyCode(e.keyCode); }), map(toFormControlValue), debounceTime(this.taDebounce), concat(), distinctUntilChanged(), filter(hasCharacters), switchMap(function (query) { return _this.suggest(query); }))
     ...
     */
    function (list, query) {
        return of$1(list.filter(function (item) { return item.includes(query); }));
    };
